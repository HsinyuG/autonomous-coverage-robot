...
bool mst_path::generate_path (geometry_msgs::Point start)
{
    ...
    // Log initial values
    ROS_INFO("Original start coordinates: x=%.2f, y=%.2f", start.x, start.y);
    ROS_INFO("Map origin: x=%.2f, y=%.2f", map.info.origin.position.x, map.info.origin.position.y);
    ROS_INFO("Map resolution: %.2f", map.info.resolution);
    ROS_INFO("Rotation: %.2f radians", rotation);
    ROS_INFO("Map width: %.2d, Map height: %.2d", map.info.width, map.info.height);
    ...
    if (start_rt.x < 0 || map.info.width < start_rt.x || start_rt.y < 0 || map.info.height < start_rt.y) {
        ROS_INFO("Start point (%.2f,%.2f) out of map!", start.x, start.y);
    }
    ...
    // safe to proceed
            ...
            ROS_INFO("Invalid node access attempted.");
            return false;
        }
        ROS_INFO("Processing node at index: %d", current);
        if (current < 0 || current >= nodes.size()) {
            ROS_INFO("Index out of bounds: %d", current);
            return false;
        }
    ...
    if (!found) {
        ROS_INFO("Could not generate coverage path, invalid starting point!");
        return false;
    }
...
nav_msgs::Path mst_path::get_path() {
    ...
    // Call add_angles() to ensure each waypoint has the correct orientation
    nav_path = add_angles();  // Make sure this function populates the poses with orientations

    // // Assuming add_angles() populates a member variable that holds PoseStamped vectors
    // for (const auto& pose_stamped : oriented_poses) { // Replace 'path' with 'oriented_poses' if necessary
    //     // Pose calculations were already handled in add_angles(), so we just use them here
...
nav_msgs::Path mst_path::add_angles()
{
    // if (path.empty()) return; // Handle empty path

    // Temporary vector to store poses with orientations
    std::vector<geometry_msgs::PoseStamped> oriented_poses;
    geometry_msgs::PoseStamped pose_stamped;

    for (size_t i = 0; i < path.size(); ++i) {
        // Get current point
        geometry_msgs::Point& current = path[i];

        // Prepare pose stamped
        pose_stamped.pose.position = current;
        pose_stamped.header.frame_id = "map";
        pose_stamped.header.stamp = ros::Time::now();

        // Compute the orientation to the next waypoint
        if (i < path.size() - 1) {
            geometry_msgs::Point& next = path[i + 1];
        ...
        pose_stamped.pose.orientation = tf2::toMsg(q);
            ...
            // Last waypoint, use the same orientation as the previous
            pose_stamped.pose.orientation = oriented_poses.back().pose.orientation;
        }

        // Add to the list of oriented poses
        oriented_poses.push_back(pose_stamped);
    }

    // Replace path with oriented poses
    nav_msgs::Path nav_path;
    nav_path.poses = oriented_poses;
    nav_path.header.frame_id = "map";
    nav_path.header.stamp = ros::Time::now();
    return nav_path;
    }
