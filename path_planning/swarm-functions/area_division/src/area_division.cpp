...
nav_msgs::OccupancyGrid area_division::get_grid (nav_msgs::OccupancyGrid map, string cps)
{
    ...
    assigned = map;
    std::vector<signed char> map_grid;
    map_grid.assign(map.data.begin(), map.data.end());
    // int width = map.info.width; == cols
    // int height = map.info.height; == rows

    for (int i=0; i<rows; i++) {
        for (int j=0; j<cols;j++) {
            // mark assigned cells as free
            int index = i * cols + j;
            signed char cell_value = map_grid[index];
            if ((A[i*cols+j] == uuid_map[cps]) && (cell_value == 0)) {
...
//  Implementation of initialize_cps function
...
void area_division::initialize_cps(std::map<std::string, std::vector<int>> cpss) {
    ...
    // Assuming gridmap is a vector of signed char initialized to a default value
    gridmap.assign(rows * cols, 0);

    // place cpss in the map
    for (const auto& c : cpss) {
        // division algorithm assumes origin at top left
        int x = c.second[0];
        int y = c.second[1];
        ...
        if (idx >= 0 && idx < static_cast<int>(gridmap.size())) {
            gridmap[idx] = std::numeric_limits<signed char>::max();
            A[idx] = nr;
            ...
            ROS_WARN("CPS position (%d, %d) is out of bounds. Skipping this CPS.", x, y);
            ROS_INFO("CPS position (%d, %d) is out of bounds. Skipping this CPS.", x, y);
        }
    }
    ROS_INFO("endupcps");
}
...
void area_division::initialize_map (int r, int c, vector<signed char> src)
{
    ...
    ROS_WARN("There are %d occupied cells.", ob);
}
